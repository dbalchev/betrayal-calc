...
export function dice3Distribution(n: number) {
    ...
    for (let perm of dice3Permutations(n)) {
        ...
        result.set(score, result.get(score) + probability)
    }
    return result
}

export function addNumberToDistribution(distribution, x) {
    let result = new Map()
...
export class DistributionSummaryEntry {
    ...
    constructor(public n :number, preSumExclusive, current, postSumInclusive) {
...
export function subtractDistributions(leftDistribution: Map<number, number>, rightDistribution: Map<number, number>) {
    let result: Map<number, number> = new Map()
    for (let [leftScore, leftProb] of leftDistribution.entries()) {
        for (let [rightScore, rightProb] of rightDistribution.entries()) {
            ...
            let oldValue = result.has(cuScore) ? result.get(cuScore) : 0.0
